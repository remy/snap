...
// Wrapper for rimraf to handle API properly
function removeDirectory(dirPath, callback) {
  rimraf(dirPath, { force: true }, callback);
}
  ...
  return new Promise((resolve, reject) => {
    ...
    fs.stat(outputPath, (statError) => {
      if (statError && statError.code === 'ENOENT') {
        ...
        resolve(crawl(url, outputPath, false));
      } else if (!statError) {
        ...
        fs.readdir(outputPath, (readError, files) => {
          ...
          if (files.length === 0) {
            ...
            rl.question(`Found ${files.length} file(s) in "${outputPath}". Delete them? (yes/no) [no] `, (answer) => {
              rl.close();

              if (answer.toLowerCase() === 'yes' || answer.toLowerCase() === 'y') {
                removeDirectory(outputPath, (error) => {
                  if (error) {
                    return reject(error);
                  }
                  resolve(crawl(url, outputPath, false));
                ...
                // User chose not to delete, merge into directory and skip existing files
                resolve(crawl(url, outputPath, true));
              }
            });
          }
        });
...
function crawl(url, outputPath, skipExisting = false) {
  return new Promise((resolve, reject) => {
    ...
    crawler.on('fetchcomplete', (queueItem, responseBuffer, response) => {
      ...
      if (skipExisting) {
        fs.exists(filepath, exists => {
          if (exists) {
            console.error('Skipped (already exists): %s', filepath);
            return;
          }
          writeFileWithDirCreation(filepath, dirname, responseBuffer);
        });
      } else {
        writeFileWithDirCreation(filepath, dirname, responseBuffer);
      }
...
function writeFileWithDirCreation(filepath, dirname, responseBuffer) {
  // Check if DIR exists
